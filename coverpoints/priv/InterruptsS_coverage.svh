...
`endif
`endif
    sie_ones:    coverpoint intr_sie_ones(ins) { bins ones = {1}; }
    ...
    sip_ones: coverpoint intr_sip_ones(ins) { bins ones = {1}; }
    ...
    xie_pair: coverpoint (ins.current.csr[CSR_SIE][15:0] & 16'h2AAA) {
        `ifdef UDB_SSI_SUPPORTED
        `ifdef UDB_STI_SUPPORTED
        bins ssi_sti = {16'h0022};
        `endif
    ...
    sie_stie_one: coverpoint ins.current.csr[CSR_SIE][5] { bins one = {1}; }

    // per-interrupt building blocks
`ifdef UDB_LCOFI_SUPPORTED
`ifdef S_SUPPORTED
    mideleg_lcofi: coverpoint ins.current.csr[CSR_MIDELEG][13] { `ifdef UDB_LCOFI_DELEGATION_SUPPORTED bins one = {1}; `endif }
    lcofi_observed: coverpoint intr_observed(ins, 13) { bins observed = {1}; }
`endif
`endif
`ifdef UDB_STI_SUPPORTED
`ifdef S_SUPPORTED
    mideleg_sti: coverpoint ins.current.csr[CSR_MIDELEG][5] { `ifdef UDB_STI_DELEGATION_SUPPORTED bins one = {1}; `endif }
    sti_observed: coverpoint intr_observed(ins, 5) { bins observed = {1}; }
`endif
`endif
`ifdef UDB_SSI_SUPPORTED
`ifdef S_SUPPORTED
    mideleg_ssi: coverpoint ins.current.csr[CSR_MIDELEG][1] { `ifdef UDB_SSI_DELEGATION_SUPPORTED bins one = {1}; `endif }
    ssi_observed: coverpoint intr_observed(ins, 1) { bins observed = {1}; }
`endif
`endif
`ifdef UDB_SEI_SUPPORTED
`ifdef S_SUPPORTED
    mideleg_sei: coverpoint ins.current.csr[CSR_MIDELEG][9] { `ifdef UDB_SEI_DELEGATION_SUPPORTED bins one = {1}; `endif }
    sei_observed: coverpoint intr_observed(ins, 9) { bins observed = {1}; }
`endif
`endif

    // cp_trigger: raise each interrupt from S, U mode
`ifdef UDB_LCOFI_SUPPORTED
`ifdef S_SUPPORTED
    cp_trigger_S_LCOFI: cross intr_priv_s, sstatus_sie, mideleg_lcofi, sie_ones, stvec_mode, lcofi_observed;
`ifdef U_SUPPORTED
    cp_trigger_U_LCOFI: cross intr_priv_u, sstatus_sie, mideleg_lcofi, sie_ones, stvec_mode, lcofi_observed;
`endif
`endif
`endif
`ifdef UDB_STI_SUPPORTED
`ifdef S_SUPPORTED
    cp_trigger_S_STI: cross intr_priv_s, sstatus_sie, mideleg_sti, sie_ones, stvec_mode, sti_observed;
`ifdef U_SUPPORTED
    cp_trigger_U_STI: cross intr_priv_u, sstatus_sie, mideleg_sti, sie_ones, stvec_mode, sti_observed;
`endif
`endif
`endif
`ifdef UDB_SSI_SUPPORTED
`ifdef S_SUPPORTED
    cp_trigger_S_SSI: cross intr_priv_s, sstatus_sie, mideleg_ssi, sie_ones, stvec_mode, ssi_observed;
`ifdef U_SUPPORTED
    cp_trigger_U_SSI: cross intr_priv_u, sstatus_sie, mideleg_ssi, sie_ones, stvec_mode, ssi_observed;
`endif
`endif
`endif
`ifdef UDB_SEI_SUPPORTED
`ifdef S_SUPPORTED
    cp_trigger_S_SEI: cross intr_priv_s, sstatus_sie, mideleg_sei, sie_ones, stvec_mode, sei_observed;
`ifdef U_SUPPORTED
    cp_trigger_U_SEI: cross intr_priv_u, sstatus_sie, mideleg_sei, sie_ones, stvec_mode, sei_observed;
`endif
`endif
`endif
...
`ifdef S_SUPPORTED
`ifdef UDB_SSI_SUPPORTED
    cp_trigger_reg_S_SSI_sip: cross csr_priv_s, csr_sstatus_sie, mideleg_ssi_one, sie_ones, stvec_mode, sip_ssip_set;
    cp_trigger_reg_S_SSI_mip: cross csr_priv_s, csr_sstatus_sie, mideleg_ssi, sie_ones, stvec_mode, mip_ssip_set;
`endif
`ifdef UDB_SEI_SUPPORTED
    cp_trigger_reg_S_SEI_mip: cross csr_priv_s, csr_sstatus_sie, mideleg_sei, sie_ones, stvec_mode, mip_seip_set;
`endif
`ifdef U_SUPPORTED
`ifdef UDB_SSI_SUPPORTED
    cp_trigger_reg_U_SSI_sip: cross csr_priv_u, csr_sstatus_sie, mideleg_ssi_one, sie_ones, stvec_mode, sip_ssip_set;
    cp_trigger_reg_U_SSI_mip: cross csr_priv_u, csr_sstatus_sie, mideleg_ssi, sie_ones, stvec_mode, mip_ssip_set;
`endif
`ifdef UDB_SEI_SUPPORTED
    cp_trigger_reg_U_SEI_mip: cross csr_priv_u, csr_sstatus_sie, mideleg_sei, sie_ones, stvec_mode, mip_seip_set;
`endif
`endif
`endif
...
`ifdef SSTC_SUPPORTED
`ifdef UDB_STI_SUPPORTED
    cp_trigger_sti_sstc_S: cross csr_priv_s, menvcfg_stce, csr_sstatus_sie, mideleg_sti, sie_ones, stimecmp_zero;
`ifdef U_SUPPORTED
    cp_trigger_sti_sstc_U: cross csr_priv_u, menvcfg_stce, csr_sstatus_sie, mideleg_sti, sie_ones, stimecmp_zero;
`endif
`endif
`endif
`endif
...
`ifdef UDB_LCOFI_SUPPORTED
`ifdef S_SUPPORTED
    cp_enable_S_LCOFI: cross intr_priv_s, sie_walk, lcofi_observed;
`ifdef U_SUPPORTED
    cp_enable_U_LCOFI: cross intr_priv_u, sie_walk, lcofi_observed;
`endif
`endif
`endif
`ifdef UDB_STI_SUPPORTED
`ifdef S_SUPPORTED
    cp_enable_S_STI: cross intr_priv_s, sie_walk, sti_observed;
`ifdef U_SUPPORTED
    cp_enable_U_STI: cross intr_priv_u, sie_walk, sti_observed;
`endif
`endif
`endif
`ifdef UDB_SSI_SUPPORTED
`ifdef S_SUPPORTED
    cp_enable_S_SSI: cross intr_priv_s, sie_walk, ssi_observed;
`ifdef U_SUPPORTED
    cp_enable_U_SSI: cross intr_priv_u, sie_walk, ssi_observed;
`endif
`endif
`endif
`ifdef UDB_SEI_SUPPORTED
`ifdef S_SUPPORTED
    cp_enable_S_SEI: cross intr_priv_s, sie_walk, sei_observed;
`ifdef U_SUPPORTED
    cp_enable_U_SEI: cross intr_priv_u, sie_walk, sei_observed;
`endif
`endif
`endif

    // cp_priority_*
    cp_priority_sip_S: cross intr_priv_s, xip_pair, sie_ones, any_taken;
    cp_priority_sie_S: cross intr_priv_s, xie_pair, sip_ones, any_taken;
`ifdef U_SUPPORTED
    cp_priority_sip_U: cross intr_priv_u, xip_pair, sie_ones, any_taken;
    cp_priority_sie_U: cross intr_priv_u, xie_pair, sip_ones, any_taken;
`endif

    // cp_wfi: WFI waits for the timer; cp_wfi_timeout: WFI with TW = 1 traps below M-mode
`ifdef SSTC_SUPPORTED
    cp_wfi_S: cross priv_mode_s, wfi, mstatus_tw_zero, csr_sstatus_sie, sie_stie_one, menvcfg_stce_one;
`endif
    cp_wfi_timeout_S: cross intr_priv_s, wfi_timeout, mstatus_tw_one, sstatus_sie, mie_mtie;
`ifdef U_SUPPORTED
    cp_wfi_timeout_U: cross intr_priv_u, wfi_timeout, mstatus_tw_one, sstatus_sie, mie_mtie;
`endif
